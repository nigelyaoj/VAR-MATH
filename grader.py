...
def math_equal(
    ...
    try:
        prediction = prediction.replace("\\dfrac", "\\frac").replace("\\tfrac", "\\frac")
        reference = reference.replace("\\dfrac", "\\frac").replace("\\tfrac", "\\frac")
    except:
        pass
    ...
    for s in [r'\left(', r"\right)", r"\left[", r"\right]", "{", "}", "(", ")", " "]:
...
def _test_math_equal():
    ...
    gt = "\\left( -\\frac{1}{3}, \\frac{22}{3} \\right)"
    pred = "-\\frac{1}{3},\\frac{22}{3}"
